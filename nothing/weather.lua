...
function conky_main()
	...
	-- Draw date/time
	local xt = widget.x + widget.width - background.padding_x
	local yt = widget.y + background.padding_y
	local dy

	_, dy = lib.draw_text(cr, fonts.text, ALIGNR, xt, yt, widget.time.date)

	yt = yt + dy + line_spacing

	_, dy = lib.draw_text(cr, fonts.time, ALIGNR, xt, yt, widget.time.time)

	-- Draw battery text
	xb = xt
	yb = yt + dy + line_spacing * 2

	_, dy = lib.draw_text(cr, fonts.text, ALIGNR, xb, yb, widget.battery.charge)

	lib.draw_text(cr, fonts.caption, ALIGNR, xb, yb + dy + line_spacing, widget.battery.status)

	-- Draw weather icon
	local xw = widget.x + background.padding_x
	local yw = widget.y + background.padding_y + line_spacing * 0.5

	if widget.weather.icon ~= '' then
		cairo_place_image(widget.weather.icon, cr, xw, yw + (fonts.weather.height - widget.weather.icon_size)/2, widget.weather.icon_size, widget.weather.icon_size, 1)
	end

	-- Draw weather temperature text
	_, dy = lib.draw_text(cr, fonts.weather, ALIGNL, xw + widget.weather.icon_size + widget.spacing_x, yw, widget.weather.temperature..'°C')

	yw = yw + dy + line_spacing * 1.25

	_, dy = lib.draw_text(cr, fonts.caption, ALIGNL, xw, yw, 'Feels like '..widget.weather.feels_like..'°C')

	-- Draw weather status text
	yw = yb

	_, dy = lib.draw_text(cr, fonts.text, ALIGNL, xw, yw, widget.weather.description)

	yw = yw + dy + line_spacing

	lib.draw_text(cr, fonts.caption, ALIGNL, xw, yw, widget.weather.location)
